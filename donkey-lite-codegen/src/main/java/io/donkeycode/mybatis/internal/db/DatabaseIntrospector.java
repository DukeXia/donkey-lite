 ...
 */
package io.donkeycode.mybatis.internal.db;

import static io.donkeycode.mybatis.internal.util.JavaBeansUtil.getCamelCaseString;
import static io.donkeycode.mybatis.internal.util.JavaBeansUtil.getValidPropertyName;
import static io.donkeycode.mybatis.internal.util.StringUtility.composeFullyQualifiedTableName;
import static io.donkeycode.mybatis.internal.util.StringUtility.isTrue;
import static io.donkeycode.mybatis.internal.util.StringUtility.stringContainsSQLWildcard;
import static io.donkeycode.mybatis.internal.util.StringUtility.stringContainsSpace;
...
import io.donkeycode.mybatis.api.JavaTypeResolver;
import io.donkeycode.mybatis.api.dom.java.FullyQualifiedJavaType;
import io.donkeycode.mybatis.api.dom.java.JavaReservedWords;
...
public class DatabaseIntrospector {
            ...
            while (tableColumns.hasNext()) {
                ...
                if (tc.isColumnIgnored(introspectedColumn.getActualColumnName())) {
                    ...
                    if (log.isDebugEnabled()) {
                        log.debug(getString("Tracing.3", //$NON-NLS-1$
        ...
        for (Map.Entry<ActualTableName, List<IntrospectedColumn>> entry : columns.entrySet()) {
            for (IntrospectedColumn introspectedColumn : entry.getValue()) {
                ...
                if (columnOverride != null) {
                    if (log.isDebugEnabled()) {
                        log.debug(getString("Tracing.4", //$NON-NLS-1$
                            introspectedColumn.getActualColumnName(), entry.getKey().toString()));
                    }
    ...
    private Map<ActualTableName, List<IntrospectedColumn>> getColumns(TableConfiguration tc) throws SQLException {
        ...
        while (rs.next()) {
            ...
            if (log.isDebugEnabled()) {
                log.debug(getString("Tracing.2", //$NON-NLS-1$
