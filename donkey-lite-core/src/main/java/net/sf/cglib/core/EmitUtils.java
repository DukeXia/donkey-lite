...
public class EmitUtils {
    ...
    private static void string_switch_trie(final CodeEmitter e, String[] strings, final ObjectSwitchCallback callback) throws Exception {
        ...
        final Map buckets = CollectionUtils.bucket(Arrays.asList(strings), (value) -> new Integer(((String) value).length()));
    ...
    private static void stringSwitchHelper(final CodeEmitter e, List strings, final ObjectSwitchCallback callback, final Label def, final Label end, final int index) throws Exception {
        ...
        final Map buckets = CollectionUtils.bucket(strings, (value) -> new Integer(((String) value).charAt(index)));
        e.dup();
        e.push(index);
    ...
    private static void load_class_helper(CodeEmitter e, final Type type) {
        if (e.isStaticHook()) {
            ...
            e.invoke_static(Constants.TYPE_CLASS, FOR_NAME, false);
        } else {
            ...
            if (!ce.isFieldDeclared(fieldName)) {
                ...
                hook.push(typeName);
                hook.invoke_static(Constants.TYPE_CLASS, FOR_NAME, false);
    ...
    private static void hash_array(final CodeEmitter e, Type type, final int multiplier, final CustomizerRegistry registry) {
        ...
        e.dup();
        e.ifnull(skip);
        EmitUtils.process_array(e, type, (type2) -> hash_code(e, type2, multiplier, registry));
        e.goTo(end);
        e.mark(skip);
        e.pop();
        e.mark(end);
    ...
    private static void hash_primitive(CodeEmitter e, Type type) {
        switch (type.getSort()) {
            ...
            case Type.FLOAT:
                ...
                e.invoke_static(Constants.TYPE_FLOAT, FLOAT_TO_INT_BITS, false);
                break;
            case Type.DOUBLE:
                // Double.doubleToLongBits(f), hash_code(Long.TYPE)
                e.invoke_static(Constants.TYPE_DOUBLE, DOUBLE_TO_LONG_BITS, false);
                // fall through
            case Type.LONG:
                hash_long(e);
                break;
	...
	*/
    ...
    private static void member_helper_size(final CodeEmitter e, List members, final ObjectSwitchCallback callback, final ParameterTyper typer, final Label def, final Label end) throws Exception {
        final Map buckets = CollectionUtils.bucket(members, (value) -> new Integer(typer.getParameterTypes((MethodInfo) value).length));
        e.dup();
        e.arraylength();
