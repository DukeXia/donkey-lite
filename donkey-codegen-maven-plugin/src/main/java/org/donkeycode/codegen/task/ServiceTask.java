...
public class ServiceTask extends AbstractTask {
	...
	public void run() throws IOException, TemplateException {
		...
		serviceData.put("ServicePackageName", path.getService());
		serviceData.put("EntityPackageName", path.getEntity());
		serviceData.put("DaoPackageName", path.getDao());
		...
		String filePath = FileUtil.getSourcePath() + StringUtil.package2Path(ConfigUtil.getConfiguration().getPackageName()) + StringUtil.package2Path(path.getService());
		String fileName;
		if (StringUtil.isBlank(path.getInterf())) { // 表示不生成Service接口文件
