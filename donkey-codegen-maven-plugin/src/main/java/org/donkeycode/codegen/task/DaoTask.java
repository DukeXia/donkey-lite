...
public class DaoTask extends AbstractTask {
	...
	public DaoTask(TableClass tableClass) {
		super(tableClass);
	}
	...
	public void run() throws IOException, TemplateException {
		...
		// 生成Dao填充数据
		log.info("Generating " + className + "Dao.java");
		Map<String, String> daoData = new HashMap<>();
		daoData.put("BasePackageName", ConfigUtil.getConfiguration().getPackageName());
		daoData.put("DaoPackageName", path.getDao());
		daoData.put("EntityPackageName", path.getEntity());
		...
		String filePath = FileUtil.getSourcePath() + StringUtil.package2Path(ConfigUtil.getConfiguration().getPackageName()) + StringUtil.package2Path(path.getDao());
